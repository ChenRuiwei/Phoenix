...
pub struct FutexFuture {
    ...
    /// addr this thread is waiting for(i.e. futex queue) through another
    /// thread. The reason why we use UnsafeCell is that since we will lock
    /// the process every time we poll, there is no need locking again.
    addr: Arc<SyncUnsafeCell<VirtAddr>>,
    expected_val: u32,
...
impl Future for FutexFuture {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        current_process().inner_handler(|proc| {
            ...
            // let addr = unsafe { *self.addr.get() };
            // if unsafe { atomic_load_acquire(addr.0 as *const u32) } !=
            // self.expected_val {     proc.futex_queue.
            // remove_waiter(addr, current_task().tid());     // TODO:
            // change thread's owned futexes when requeue     unsafe
            // {         (*current_task().inner.get()).
            // owned_futexes.0.insert(addr);     }
            //     Poll::Ready(())
            // } else {
            //     Poll::Pending
            // }
