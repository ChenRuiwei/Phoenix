...
pub fn handle_exit(thread: &Arc<Thread>) {
    ...
    // The reason why we clear tid addr here is that in the destruction function of
    // TidAddr, we will lock the process inner.
...
/// Terminate all threads of the current process except main thread(i.e. idx =
/// 0). Note that the caller cannot hold the process inner's lock
