...
use config::process::CLONE_STACK_SIZE;
pub use manager::{PROCESS_GROUP_MANAGER, PROCESS_MANAGER};
use systype::{GeneralRet, SyscallErr, SyscallRet};
use thread::Thread;
...
impl Process {
    ...
    pub fn new_initproc(elf_data: &[u8], elf_file: Option<&Arc<dyn File>>) -> Arc<Self> {
        ...
        thread::spawn_user_thread(thread);
        debug!("create a new process, pid {}", process.pid());
        process
    }

    /// Fork a new process
    /// `stack` points to the new cloned process's main thread's stack if not
    /// `None`
    pub fn fork(
        self: &Arc<Self>,
        stack: Option<usize>,
    ...
    /// When one process invokes `exec`, all of the threads will terminate
    /// except the main thread, and the new program is executed in the main
    /// thread.
    pub fn exec(
        &self,
        elf_data: &[u8],
        ...
        let (memory_space, ustack_top, entry_point, mut auxv) =
            MemorySpace::from_elf(elf_data, elf_file);
        ...
        auxv.push(AuxHeader::new(AT_EXECFN, argv[0])); // file name
        auxv.push(AuxHeader::new(AT_NULL, 0)); // end

        stack_trace!();
        // Construct auxv
        debug!("auxv len {}", auxv.len());
        let len = auxv.len() * core::mem::size_of::<AuxHeader>();
        user_sp -= len;
        UserCheck::new().check_writable_slice(user_sp as *mut u8, len)?;
        let auxv_base = user_sp;
        for i in 0..auxv.len() {
            unsafe {
                // *((user_sp + i * core::mem::size_of::<AuxHeader>()) as *mut AuxHeader) =
                // auxs[i];
                *((user_sp + i * core::mem::size_of::<AuxHeader>()) as *mut usize) =
                    auxv[i].aux_type;
                *((user_sp + i * core::mem::size_of::<AuxHeader>() + core::mem::size_of::<usize>())
                    as *mut usize) = auxv[i].value;
            }
        }
        stack_trace!();
        // Construct envp
    ...
    fn clone_thread(
        ...
        thread::spawn_user_thread(new_thread);
    ...
    fn clone_process(
        ...
        let child = self.inner_handler(move |parent_inner| {
            ...
            // let memory_space =
            // MemorySpace::from_existed_user(&parent_inner.memory_space);
        ...
        thread::spawn_user_thread(main_thread);
        Ok(child)
    }
}
