...
use driver::{getchar, CHAR_DEVICE};
use sync::{
    cell::SyncUnsafeCell,
    mutex::{SleepLock, SpinLock},
};
        ...
        #[cfg(all(not(feature = "submit"), feature = "board_u740"))]
        {
            ...
            if c == 0xff {
                if let Some(waker) = waker {
                    ...
                    // POLL_QUEUE.register(
                    //     self.metadata().inner.lock().file.as_ref().unwrap().
                    // clone(),     waker,
                    //     true,
                    // )
                }
    ...
    fn ioctl(&self, command: usize, value: usize) -> SyscallRet {
        ...
        match command {
            TCGETS | TCGETA => {
                ...
                unsafe {
                    // (value as *mut Termios).copy_from(&self.inner.lock().termios as *const
                    // Termios, 1);
