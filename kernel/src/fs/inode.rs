...
use log::debug;
use sync::{cell::SyncUnsafeCell, mutex::SleepLock};
use systype::{AgeneralRet, GeneralRet, SyscallErr};

use super::{
    devfs::r#loop::FileBlockDeviceWrapper,
    file::{DefaultFile, FileMeta, FileMetaInner},
    File, Mutex, FILE_SYSTEM_MANAGER,
};
use crate::{
    fs::{HashKey, PageCache},
    stack_trace,
    timer::ffi::TimeSpec,
    utils::path,
};
...
pub enum InodeState {
    /// init, the inode may related to an inode in disk, but not load data from
    /// disk
    Init = 0x1,
...
pub trait Inode: Send + Sync {
    ...
    /// Note that this method only sync this inode itself, not including its
    /// children.
...
impl dyn Inode {
    ...
    /// This method will delete the inode in cache (which means deleting inode
    ...
    /// Unlink the child.
    /// This method will delete the inode in inode cache and call delete()
    /// function to delete inode in disk.
    ...
    /// Sync this inode.
    /// If the inode is a dir, sync it's metadata and all of its children
    /// recursively. If the inode is a regular file, sync its content.
    // #[async_recursion]
