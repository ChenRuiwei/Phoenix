...
pub use ctx::{SignalContext, SignalTrampoline};
pub use handler::{SIG_DFL, SIG_ERR, SIG_IGN};
...
pub fn check_signal_for_current_task() -> bool {
        ...
        current_task().sig_queue.lock().check_signal()
    {
        ...
        // Note that serveral sig handlers may be executed at the same time by different
        // threads since we don't hold the process inner lock
