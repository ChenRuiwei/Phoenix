...
use arch::interrupts::{set_trap_handler, InterruptGuard};
use async_utils::block_on;
use config::{mm::PAGE_SIZE, process::SYSCALL_STR_ARG_MAX_LEN};
use log::{debug, error, warn};
use memory::VirtAddr;
use riscv::register::scause::Scause;
use systype::{GeneralRet, SyscallErr};

use super::memory_space;
use crate::{
...
impl UserCheck {
    ...
    pub fn new() -> Self {
        ...
        let ret = Self {
            ...
            _sie_guard: InterruptGuard::new(),
        };
        set_trap_handler(__try_access_user_error_trap as usize);
        ret
    }
    ...
    pub fn check_c_str(&self, c_str: *const u8) -> GeneralRet<()> {
        ...
        loop {
            ...
            } else if self.check_c_str_end(va) {
                return Ok(());
            }
            va.0 += PAGE_SIZE;
