use core::{cell::RefCell, str, sync::atomic::AtomicBool};
...
use super::queue::{Local, Steal};

const WORKER_TICK_INTERVAL: u32 = 17;

pub struct Worker {
    run_queue: Local<Runnable>,
...
/// 所有的工作线程（或处理器）共享同一个Executor实例
pub struct Executor {
    injector: SegQueue<Runnable>,
    stealers: Box<[Steal<Runnable>]>,
