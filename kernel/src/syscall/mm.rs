...
use memory::{MapPermission, VPNRange, VirtAddr};
    ...
    if flags.contains(MmapFlags::MAP_ANONYMOUS) {
        ...
        current_process().inner_handler(|proc| {
            let mut vma = {
                ...
                // VmAreaType::Mmap)?         .ok_or(SyscallErr::ENOMEM)?
                // } else {
                proc.memory_space
...
pub fn sys_munmap(addr: usize, length: usize) -> SyscallRet {
    ...
    current_process().inner_handler(|proc| {
        ...
        // TODO: maybe we should check wether the user owns the permission to unmap the
        // vma?
