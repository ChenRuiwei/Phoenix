...
use core::{ops::Add, ptr, ptr::copy_nonoverlapping, time::Duration};

use async_utils::{Select2Futures, SelectOutput};
use config::{fs::PIPE_BUF_CAPACITY, mm::PAGE_SIZE};
...
use crate::{
    fs::{
        ffi::{
            Dirent, FdSet, Iovec, StatFlags, Statfs, Sysinfo, UtsName, FD_SET_LEN, SEEK_CUR,
            SEEK_END, SEEK_SET, STAT, STATFS_SIZE, STAT_SIZE, SYSINFO_SIZE, UTSNAME_SIZE,
        },
        file_system::FsDevice,
        inode,
        inode::INODE_CACHE,
        pipe::make_pipe,
        resolve_path_ffi, FaccessatFlags, FcntlFlags, FdInfo, FileSystemType, HashKey, Inode,
        InodeMode, OpenFlags, Renameat2Flags, SeekFrom, AT_FDCWD, FILE_SYSTEM_MANAGER,
    },
    ...
    syscall::PollEvents,
    timer::{
        ffi::{current_time_spec, TimeSpec},
        io_multiplex::{IOMultiplexFormat, IOMultiplexFuture, RawFdSetRWE},
        timeout_task::{TimeoutTaskFuture, TimeoutTaskOutput},
        UTIME_NOW, UTIME_OMIT,
    },
    utils::{
        path::{self, is_relative_path},
        string::c_str_to_string,
    },
};
...
pub fn sys_unlinkat(dirfd: isize, path: *const u8, flags: u32) -> SyscallRet {
    stack_trace!();
    let (target_inode, ..) = path::path_to_inode_ffi(dirfd, path)?;
    stack_trace!();
    ...
    if target.is_some() {
        ...
        match parent.metadata().mode {
            InodeMode::FileDIR => {
                ...
                // TODO: add to dirty list, should add inode to the target fs which is include
                // this inode
                drop(inner_lock);
                stack_trace!();
...
/// you should insert the target_path and filesystem into the
/// FILE_SYSTEM_MANAGER. the filesystem should be converted from block_dev which
/// is associated with the dev_name.
pub fn sys_mount(
    dev_name: *const u8,
...
/// fstat() function return information about a file, in the buffer pointed to
/// by stat_buf. This function except that the file about which information is
/// to be retrieved is specified by the file descriptor fd.
pub fn sys_fstat(fd: usize, stat_buf: usize) -> SyscallRet {
    stack_trace!();
    let _sum_guard = SumGuard::new();
    UserCheck::new().check_writable_slice(stat_buf as *mut u8, STAT_SIZE)?;
    // if !file.readable() {
    //     info!("[_fstat]: file cannot be read, fd {}, stat_buf addr {:#x}", fd,
    // stat_buf);     return Err(SyscallErr::EACCES);
    // }
...
/// We should give the stat_buf which has already been checked to _fstat
/// function.
...
pub fn sys_newfstatat(
    ...
    let _sum_guard = SumGuard::new();
    UserCheck::new().check_writable_slice(stat_buf as *mut u8, STAT_SIZE)?;
    let (inode, ..) = path::path_to_inode_ffi(dirfd, pathname)?;
    if inode.is_some() {
        // find inode
...
/// If oldpath and newpath are existing hard links referring to the same inode,
/// then return a success. If newpath exists but operation failed (for some
/// reason, rename() failed), leave an instance of newpath in place (which means
/// you should keep the backup of newpath if it exist). If oldpath can specify a
/// directory, then newpath should be a blank directory or not exist.
pub fn sys_renameat2(
    ...
    let (oldinode, ..) = path::path_to_inode_ffi(olddirfd, oldpath)?;
    ...
    if newinode.is_none() {
        // newpath doesn't exist, so we can create one and needn't care about the
        // replace problem.
        ...
        if flags.contains(Renameat2Flags::RENAME_NOREPLACE) {
            ...
            if flags.contains(Renameat2Flags::RENAME_EXCHANGE) {
                ...
                // But you should check the newpath type, if newtype is not the same as oldtype,
                // should return EISDIR or ENOTEMPTY or ENOTDIR
