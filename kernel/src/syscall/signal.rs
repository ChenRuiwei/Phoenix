use config::{process::INITPROC_PID, signal::SIG_NUM};
...
/// pid == 0 then sig is sent to every process in the process group of current
/// process
///
/// pid == -1 then sig is sent to every process which current process has
/// permission ( except init proc )
///
