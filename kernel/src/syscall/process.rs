...
use crate::{
    ...
    process,
    process::{
        thread::{exit_and_terminate_all_threads, terminate_given_thread},
        PROCESS_GROUP_MANAGER, PROCESS_MANAGER,
    },
    processor::{current_process, current_task, current_trap_cx, SumGuard},
    stack_trace,
    timer::current_time_duration,
    utils::{path, string::c_str_to_string},
};
    ...
    fn poll(
        ...
        if let Some((child_pid, exit_code)) = process.inner_handler(|proc| {
            ...
            if self.exit_status_addr != 0 {
                ...
                // TODO: here may cause some concurrency problem between we user_check and write
                // it
                let _sum_guard = SumGuard::new();
                ...
                // info!("waitpid: write pid to exit_status_ptr before, addr {:#x}",
                // exit_status_addr);
                unsafe {
