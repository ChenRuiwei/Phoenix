...
use super::{ip_packet::*, socket_set::SocketSet};
...
pub(crate) struct Fragmenter {
    ...
    /// 数据包的总长度，不包括IEEE802.15.4头部和分片头部
    packet_len: usize,
...
#[cfg(feature = "proto-sixlowpan-fragmentation")]
/// 6LoWPAN是一种使IPv6能够在低功耗、资源受限的无线网络中运行的协议。
/// 它通过数据压缩、分片重组等技术，适应了低功耗设备的需求
...
impl Interface {
    ...
    /// locally on IPv4 addresses other than the interface's configured
    ...
    /// 用于处理网络设备上的数据包接收和处理。
    /// 它接受一个设备对象device和一个socket集合sockets，并返回一个布尔值，
    /// 表示是否处理了任何数据包。
        ...
        D: Device + ?Sized,
    {
        ...
        // 使用while let语法从设备接收数据包，如果成功接收，
        // 则返回接收和发送令牌（rx_token和tx_token）
    ...
    /// 处理数据包从套接字向设备的传出。它检查哪些套接字允许发送数据，
    /// 并通过设备发送这些数据包。
        ...
        D: Device + ?Sized,
    {
        ...
        for item in sockets.items_mut() {
            ...
            match result {
                // 如果设备缓冲区已满（Exhausted）则退出循环
                Err(EgressError::Exhausted) => break,
                // 如果分发错误（Dispatch），更新邻居缓存缺失信息
...
impl InterfaceInner {
    ...
    /// 用于处理和发送IP数据包。它支持不同的网络媒介（如Ethernet和IEEE802.15.
    /// 4），并根据特定条件（如是否需要IP分片）处理数据包
