...
//             device: None,
//             pages:
// LruCache::new(NonZeroUsize::new(MAX_BUFFER_PAGES).unwrap()),
...
//                 if let Some(page) =
// self.pages.get_mut(&block_page_id(block_id)) {                     //
// log::error!("has page");                     device.base_read_block(block_id,
// page.block_range(block_id));
// page.set_buffer_head(buffer_head.clone());                 } else {
//                     // log::error!("page init");
//                     let mut page = Arc::new(Page::new_buffered());
//                     device.base_read_block(block_id,
// page.block_range(block_id));
